...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $total_wins = Winner::where('user_id', $user->id)->count();
        $totalTransaction = Transaction::where('user_id',$user->id)->count();

        return view('admin.users.detail', compact('page_title', 'user','totalDeposit','total_bids', 'total_wins', 'totalTransaction'));
    }
    ...
    //User bids
    public function bidsList($user_id)
    {
        $user = User::findOrFail($user_id);

        $page_title = 'Bids: ' . $user->username;
        $empty_message = 'No result found';
        $bids = Bid::where('user_id', $user_id)->with(['user', 'product'])->latest()->paginate(getPaginate());
        return view('admin.bids.bids-list', compact('page_title', 'bids', 'empty_message'));
    }

    // Wins
    public function allWins($user_id)
    {
        $user = User::findOrFail($user_id);

        $page_title = 'Winners & Items: '. $user->username;
        $empty_message = 'No result found';
        $winners = Winner::where('user_id', $user_id)->with(['user', 'bid'])->latest()->paginate(getPaginate());
