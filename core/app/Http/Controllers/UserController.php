...
class UserController extends Controller
{
    ...
    public function home()
    {
        ...
        $user = \auth()->user();
        $widget['current_balance'] = getAmount($user->balance);
        $widget['total_deposit'] = getAmount($user->deposits()->sum('amount'));
        $widget['total_bid'] = $user->bids()->count();
        $widget['total_bid_amount'] = $user->bids()->sum('total_amount');
        $widget['win_products'] = $user->winProducts()->count();
        $widget['total_transactions'] = $user->transactions()->count();
        $latest_transactions = Transaction::where('user_id', \auth()->id())->latest()->take(10)->get();
        return view($this->activeTemplate . 'user.dashboard', compact('page_title', 'widget', 'latest_transactions'));
    }
    ...
    public function transactions()
    {
        ...
        $logs = Transaction::where('user_id', \auth()->id())->latest()->paginate(getPaginate());
        return view($this->activeTemplate . 'user.transactions', compact('page_title', 'logs'));
    }
